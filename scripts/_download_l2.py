"""SFTP-download the L2 sequence tar with progress.
...
REMOTE = "/root/autodl-tmp/paper_eval_l2_sem_rich.tar"
LOCAL = r"D:\Users\yangwan\mProject\CARLA-Air\air_ground_relay_collect\samples\paper_eval_l2_sem_rich.tar"
...
print(f"Remote size: {total/1e9:.2f} GB", flush=True)
...
print(f"DONE in {elapsed:.0f}s ({total/elapsed/1e6:.1f} MB/s avg) -> {LOCAL}", flush=True)
sftp.close()
client.close()
